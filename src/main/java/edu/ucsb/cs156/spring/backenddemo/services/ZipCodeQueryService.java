...
import lombok.extern.slf4j.Slf4j;//op
import org.springframework.boot.web.client.RestTemplateBuilder;//
...
public class ZipCodeQueryService {
    ...
    public String getJSON(String zipcode) throws HttpClientErrorException {
       log.info("zipcode={}", zipcode);//op
        HttpHeaders headers = new HttpHeaders();
        headers.setAccept(List.of(MediaType.APPLICATION_JSON));
        headers.setContentType(MediaType.APPLICATION_JSON);
        HttpEntity<String> entity = new HttpEntity<>("body", headers);//difference

        Map<String, String> uriVariables = Map.of("zipcode", zipcode);//op
